// * FEHLER BEHOBEN. *EINGEBAUT*. KARTENFARBEN. Baue ich ein, sobald mein Spiel wieder läuft. Estimated 1-2 fehler behebung.
// * *GELÖST* Der Index zeigt immer die Farbe der vorherigen Karte an.
// * FIXED wenn reverse, skip funktion falsch
// * FIXED drawtwo noch fehlerhaft
// * FIXED ab einem gewissen punkt. werden immer weiter "Karten gezogen"
// * FIXED Spielstart fehlerhaft. trotz nein eingabe geht es einen schritt weiter.
// * FIXED Mehr Printlines zur Information der Regeln.
// ! Feedback falls falsche Kartenlegung *evlt die Person nächste Runde überspringen* / INFO
// ? Kosmetische Verbesserung Wert. Vor KartenWert schreiben
// * Kosmetische verbesserung. Kommentare über den Code schreiben *DONE*
// * Player klasse erstellen statt in der UnoGame.kt bsp name hand. evtl punkte Stand Mehr funktionen ( kampschrei ) *DONE*
// ! stats funktion *keine momentane verwendung*
// * user / KIuser erbt von player *PLAYER&NPCPLAYER in meinem fall. *DONE*
// * IF Abfrage für die Eingabe des Indexes der Maximalen Karten auf der Hand. Chosecard *DONE*
// * ab einem gewissen Punkt, wird in endlos schleife ein fehler ausgeworfen, der mich immer wieder karten ziehen lässt und alle runden überpsringt. *DONE*
// * Lag an der drawCard funktion..*
// ? deck & stack als klassen ... Auf später verschoben, einfach zum spaß das Spiel bearbeiten.
// * FIXED reverse als erste karte führt nicht den geweünschten effekt aus
// * skip als erste karte führt nicht den geweünschten effekt aus *DONE*
// ! TODO NOW! npcplayer class erbend vom player erstellen mit spiellogic vom normalem game ! // *DONE*
// * ^statt readln mit if else arbeiten *DONE*
// ! evlt karten nicht sichtbar machen *FUTURE ADD*

// ! TODO in near feature if enough time/after fulfilling project requierments. ! //
// ! TODO add class DECK&STACK for project improvement ! //
// ! TODO add NPC class for project improvement ! //



